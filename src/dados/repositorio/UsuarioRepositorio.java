...
public class UsuarioRepositorio extends GerenciaArquivos implements SuperDataInterface<Usuario> {
    ...
    public Usuario encontraPeloCPFeSenha(String cpf, int password) {
        List<Usuario> todos = findAll();
        for (Usuario usuario : todos) {
            if (usuario.getCpf().equals(cpf) && usuario.getSenha() == password) {
                return usuario;
            }
        }
    ...
    public Usuario converteMapParaEntidade(Map<String, String> linha) {
        ...
        if (tipo.equalsIgnoreCase(TipoUsuario.BANCARIO.getTipo())) {
            return new Bancario(
                    linha.get("nome"),
                    linha.get("email"),
        ...
        if (tipo.equalsIgnoreCase(TipoUsuario.GERENTE.getTipo())) {
            return new Gerente(
                    linha.get("nome"),
                    linha.get("email"),
                    Integer.parseInt(linha.get("senha")),
        ...
        if (tipo.equalsIgnoreCase(TipoUsuario.CORRENTISTA.getTipo())) {
        ...
        throw new ErroBanco("Tipo de Usuário desconhecido");
    }
